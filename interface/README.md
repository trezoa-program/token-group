...
to be considered a "token-group" program for TPL token mints. The interface can
...
This interface is compatible with any program that implements the TPL Token
interface. However, other program implementations that are not TPL Token
programs may still be compatible with an TPL Token Group program should that
...
An example program demonstrating how to implement the TPL Token-Group Interface
can be found in the
[example](https://github.com/trezoa-program/token-group/tree/main/program)
directory alongside this interface's directory.

In addition to demonstrating what a token-group program might look like, it
also provides some reference examples for using the TPL Type Length Value
library to manage TLV-encoded data within account data.

For more information on TPL Type Length Value you can reference the library's
[source code](https://github.com/trezoa-program/libraries/tree/main/type-length-value).
...
- [`InitializeGroup`](https://github.com/trzledgerfoundation/trezoa-program-library/blob/master/token-group/interface/src/instruction.rs#L22)
- [`UpdateGroupMaxSize`](https://github.com/trzledgerfoundation/trezoa-program-library/blob/master/token-group/interface/src/instruction.rs#L33)
- [`UpdateGroupAuthority`](https://github.com/trzledgerfoundation/trezoa-program-library/blob/master/token-group/interface/src/instruction.rs#L42)
- [`InitializeMember`](https://github.com/trzledgerfoundation/trezoa-program-library/blob/master/token-group/interface/src/instruction.rs#L51)
...
Must provide an TPL token mint and be signed by the mint authority.
...
Must provide an TPL token mint for both the group and the group member.
...
/// First 8 bytes of `hash("tpl_token_group_interface:group")`
...
/// First 8 bytes of `hash("tpl_token_group_interface:member")`
...
[tpl-type-length-value repo](https://github.com/trezoa-program/libraries/tree/main/type-length-value).
